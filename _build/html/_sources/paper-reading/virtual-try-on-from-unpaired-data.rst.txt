...
Similar to the Pix2PixHD approach, the generator :math:`G` is conditioned on a semantic segmentation map and on an appearance map generated by an encoder :math:`E`. The autoencoder assigns to each semantic region in the segmentation map a low-dimensional feature vector representing the region appearance. These appearance-based features enable control over the appearance of the output image and address the lack of diversity.
...
.. image:: figures/virtual-try-on-from-unpaired-data-4.png
   :width: 320pt

Main contributions
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

1. A new virtual try-on framework, Outfit-VITON
2. An inexpensive data collection and training process that uses unpaired data
3. An online optimization for fine garment features like textures, logos and embroidery

Method Overview
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

1. **Shape generation:** generating a segmentation map that combines the sihouttes of the garments with the segmentation map of the query image
   - A PSP semantic segmentation model is used to segment the query and reference images into :math:`D_c` classes which include body parts and garment types
   - A DensePose network is used to capture the pose and body shape of humans :math:`b`
   - A shape autoencoder encodes the one-hot segmentation of :math:`H \times W \times D_c` into :math:`8 \times 4 \times D_s` dimensions
   - The shape generator is given by :math:`s^y = G_{shape}(b, e^s)`
2. **Appearance generation:** generating a photo-realistic image showing the person in the query image dressed with the selected garments
   - An appearance encoder that encode appearance features into :math:`H \times W \times D_t` and then pooling to :math:`1 \times D_t`
   - The appearance generator is given by :math:`y = G_{app}(s^y, e^t)`
3. **Online optimization:** refine the appearance
...
.. image:: figures/virtual-try-on-from-unpaired-data-6.png
   :width: 400pt

Let a training pair be :math:`x^0 + (x^1, x^2)` where :math:`x^0` is the query image and :math:`x^1`, :math:`x^2` are two reference images. The output of the framework would be:

.. math::

   y = G_{app}(G_{shape}(x^0, (x^1, x^2)))

We should further enforce that

.. math::

   x^1 & = G_{app}(G_{shape}(x^0, (y, x^2))) \\
   x^2 & = G_{app}(G_{shape}(x^0, (x^1, y)))
